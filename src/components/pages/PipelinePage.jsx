...
export default function PipelinePage() {
  const { scenarios, loading, triggering, fetchScenarios, triggerScenario } = useMakeScenarios();
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'24px 28px' }}>
        {!hasMakeKey && (
          <div style={{ background:'#1A130A', border:'1px solid #F59E0B33', borderRadius:8, padding:'10px 14px', fontSize:12, color:'#8A7040', marginBottom:20, lineHeight:1.6 }}>
            ⚠ Connect Make.com by setting <code style={{ color:'#F59E0B', background:'#2A1F08', borderRadius:3, padding:'1px 5px' }}>REACT_APP_MAKE_API_KEY</code> and <code style={{ color:'#F59E0B', background:'#2A1F08', borderRadius:3, padding:'1px 5px' }}>REACT_APP_MAKE_TEAM_ID</code> in Vercel environment variables. Showing demo data.
          </div>
        )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {scenarios.map(s => {
            ...
            return (
              <div key={s.id} style={{ background:'#0D0F17', border:'1px solid #1A1D26', borderRadius:10, padding:'15px 18px', display:'flex', alignItems:'center', gap:14 }}>
                {/* Status dot */}
